...
def plot(case):
    ...
    if case==0:
        ...
        plt.subplots_adjust(left=0.04, bottom=0.1, right=0.97, top=0.95, hspace=0.25, wspace=0.26)
        plt.subplot(131)
        ...
        plt.legend(loc=1,fontsize=labelsize, frameon=False,handlelength=1.0,bbox_to_anchor=[1.0,0.95])
        ...
        def plot2(data,color,label):
            ...
            plt.ylim(-1.0,0.6)
            plt.xlim(-0.7,0.6)
            plt.yticks([-0.9,0.5],fontsize=fontsize)
            plt.xticks([-0.5, 0.5],fontsize=fontsize)
        plot2(data[1], colors[0], 'FESSNC')
        plot2(data[2], colors[1], 'GP-MPC')
        plot2(data[3], colors[-1], 'BALSA')
        plt.legend(loc=3, fontsize=labelsize, frameon=False,handlelength=1.0,bbox_to_anchor=[0.01,-0.08])
        ...
        plt.legend(loc=3, fontsize=labelsize, frameon=False,handlelength=1.2,bbox_to_anchor=[0.25,-0.08],handletextpad=0.3)
        plt.xlabel('Trial', fontsize=fontsize, labelpad=-15)
        plt.ylabel('Success Rate $\%$', fontsize=fontsize, labelpad=-30)
