...
class ICNN(nn.Module):
    ...
    def calcul_lip(self):
        ...
        for i in range(len(self._ws)):
            sub_lip = torch.linalg.norm(self._ws[i],ord=2)
            for j in range(i-1,len(self._us)):
                sub_lip *= torch.linalg.norm(self._us[j],ord=2)
            lip += sub_lip
        return lip
...
class NormalControlNet(torch.nn.Module):
    ...
    def calcul_lip(self):
        ...
        lip = torch.linalg.norm(W1,ord=1)*torch.linalg.norm(W2,ord=1)*torch.linalg.norm(W3,ord=1)
        return lip

class NormalVNet(torch.nn.Module):

    def __init__(self, n_input, n_hidden, n_output):
        super(NormalVNet, self).__init__()
        torch.manual_seed(2)
        self.layer1 = (torch.nn.Linear(n_input, n_hidden))
        self.layer2 = (torch.nn.Linear(n_hidden, n_hidden))
        self.layer3 = (torch.nn.Linear(n_hidden, n_output))
...
def ZBF(x):
    # -4pi/3<theta1-pi<pi/3
    outer = 2.
    inner = 1.
    # constranit = torch.cat((outer**2 - (x[:,0:1]**2+x[:,1:2]**2),(x[:,0:1]-2.)**2+x[:,1:2]**2-inner**2),dim=1)
    # return torch.min(constranit,dim=1)[0]
    return outer**2-torch.sum(x[:,0:2]**2,dim=1)
...
class NormalLyapunovFunction(nn.Module):
    '''
    This class is used to train the controller without spectral regularization
    '''
    def __init__(self, n_input, n_hidden, n_output, input_shape, smooth_relu_thresh=0.1, layer_sizes=[64, 64],
                 eps=1e-3):
        super(NormalLyapunovFunction, self).__init__()
        torch.manual_seed(2)
        self._d = smooth_relu_thresh
        self._icnn = ICNN(input_shape, layer_sizes, self.smooth_relu)
        self._eps = eps
        self._control = NormalControlNet(n_input, n_hidden, n_output)
    ...
    def forward(self, x):
        ...
        return self.smooth_relu(g - g0) + self._eps * x.pow(2).sum(dim=1), u-u0 #u * x
        # return self.smooth_relu(g - g0) + self._eps * x.pow(2).sum(dim=1), u-u0
...
class RSMFunction(nn.Module):
    '''
    This class is used to train the controller without spectral regularization
    '''
    def __init__(self, n_input, n_hidden, n_output,input_shape, smooth_relu_thresh=0.1, layer_sizes=[64, 64],eps=1e-3,mode=1):
        super(RSMFunction, self).__init__()
        torch.manual_seed(2)
        self._eps = eps
        self._d = smooth_relu_thresh
        self.mode = mode
        if self.mode == 1:
            self._V = NormalVNet(n_input, n_hidden, 1)
        if self.mode == 2:
            self._V = ICNN(input_shape, layer_sizes, self.smooth_relu)
        self._control = ControlNet(n_input, n_hidden, n_output)


    def forward(self, x):
        g = self._V(x)
        g0 = self._V(torch.zeros_like(x))
        u = self._control(x)
        u0 = self._control(torch.zeros_like(x))
        if self.mode == 1:
            return self._V(x)**2, u * x #u-u0 #u * x
        if self.mode == 2:
            return self.smooth_relu(g - g0).view(-1,1) + (self._eps * x.pow(2).sum(dim=1)).view(-1,1), u * x #u-u0 #u * x
